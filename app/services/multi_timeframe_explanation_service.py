...
def _inspect(df: pd.DataFrame, label: str) -> TimeframeInsight:
    ...
    if len(data) < 35:
        raise ValueError(f"{label} için yeterli mum yok")
...
def build_multi_timeframe_package(provider, symbol: str, *, timezone_name="Europe/Istanbul", now=None):
    ...
    for label, interval, lookback in (("5 dk", "5m", 8), ("15 dk", "15m", 25), ("1 saat", "1h", 120)):
        frames[label] = provider.get_ohlcv(symbol, interval, end - timedelta(days=lookback), end)
    frames["4 saat"] = resample_completed_4h(frames["1 saat"], now=end, timezone_name=timezone_name)
    insights, errors = [], []
